...
#
select_model <- function(loo_results) {
  if (rownames(loo_results)[1] == 'model1') {
    return(1)
  } else if (rownames(loo_results)[1] == 'model2') {
    if (abs(loo_results['model1',][['elpd_diff']]) < 2 * loo_results['model1',][['se_diff']]) {
      return(1)
    } else {
      return(2)
    }
    ...
    if (abs(loo_results['model1',][['elpd_diff']]) < 2 * loo_results['model1',][['se_diff']]) {
      return(1)
    } else if (abs(loo_results['model2',][['elpd_diff']]) < 2 * loo_results['model2',][['se_diff']]) {
      return(2)
    } else {
      return(3)
    }
    ...
    if (abs(loo_results['model1',][['elpd_diff']]) < 2 * loo_results['model1',][['se_diff']]) {
      ...
      return(2)
    } else if (abs(loo_results['model3',][['elpd_diff']]) < 2 * loo_results['model3',][['se_diff']]) {
      return(3)
    } else {
      return(4)
    }
